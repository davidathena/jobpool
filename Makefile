...
#FINAL_TAG = 2.0.2-SNAPSHOT.x86_64
FINAL_TAG = GA-2.0.2.x86_64
# -------------配置项（打正式镜像的时候写死）-------------
...
build:
	...
	./bin/jobpool --version
	./bin/jobpoolctl version
	./bin/jobpool_agent --version
...
compile-with-docker-test:
	...
	docker run \
	  ...
	  /bin/bash -c "GO_BUILD_FLAGS=-v GOOS=linux GOARCH=amd64 ./build.sh && ./bin/jobpool --version"
...
compile-setup-gopath-with-docker-test:
	...
	docker run \
	  ...
	  gcr.io/etcd-development/etcd-test:go$(GO_VERSION) \
	  /bin/bash -c "cd /etcd && ETCD_SETUP_GOPATH=1 GO_BUILD_FLAGS=-v GOOS=linux GOARCH=amd64 ./build.sh && ./bin/jobpool --version && rm -rf ./gopath"
...
# Example:
#   make build-docker-release-main
#   make build-docker-release-agent
...
build-docker-release-main:
	...
	cp ./jobpool.sh ./bin/jobpool.sh
	docker build \
	  --network=host \
	  --tag registry.cn-beijing.aliyuncs.com/yunli-data-platform/jobpool-v2:$(ETCD_VERSION) \
	  --file ./bin/Dockerfile-release.$(ARCH) \
	  ./bin
	docker tag \
		registry.cn-beijing.aliyuncs.com/yunli-data-platform/jobpool-v2:$(ETCD_VERSION) \
		registry.cn-beijing.aliyuncs.com/yunli-data-platform/jobpool-v2:$(FINAL_TAG)
	rm -f ./bin/Dockerfile-release.$(ARCH)
	docker push registry.cn-beijing.aliyuncs.com/yunli-data-platform/jobpool-v2:$(FINAL_TAG)
...
build-docker-release-agent:
	...
	docker build \
    	  ...
    	  ./bin
	docker tag \
		registry.cn-beijing.aliyuncs.com/yunli-data-platform/jobpool-client-v2:$(ETCD_VERSION) \
		registry.cn-beijing.aliyuncs.com/yunli-data-platform/jobpool-client-v2:$(FINAL_TAG)
	rm -f ./bin/Dockerfile-release.$(ARCH)
	docker push registry.cn-beijing.aliyuncs.com/yunli-data-platform/jobpool-client-v2:$(FINAL_TAG)
...
build-docker-functional:
	...
	docker run \
	  ...
	  /bin/bash -c "./bin/jobpool --version && \
