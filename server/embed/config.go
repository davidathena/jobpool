...
const (
	...
	DefaultWorkerNumbers         = 1
	DefaultJobParallelismLimit   = 100
...
type Config struct {
	...
	// scheduler and something
	WorkerNumber int `json:"worker-number"`

	// 任务并行度
	JobParallelismLimit int `json:"job-parallelism-limit"`
	...
	MinHeartbeatTTLCfg string `json:"min-heartbeat-ttl"`
	// 认为已下线的ttl
...
func NewConfig() *Config {
	...
	cfg := &Config{
		...
		WorkerNumber:        DefaultWorkerNumbers,
		JobParallelismLimit: DefaultJobParallelismLimit,
		...
		MinHeartbeatTTLCfg:         "10s",
		FailoverHeartbeatTTLCfg:    "120s",
...
func (cfg *configYAML) configFromFile(path string) error {
	...
	if cfg.MinHeartbeatTTLCfg != "" {
		cfg.MinHeartbeatTTL, err = time.ParseDuration(cfg.MinHeartbeatTTLCfg)
		if err != nil {
			fmt.Fprintf(os.Stderr, "unexpected error setting up min-heartbeat-ttl: %v\n", err)
			os.Exit(1)
		}
	}
	if cfg.FailoverHeartbeatTTLCfg != "" {
		cfg.FailoverHeartbeatTTL, err = time.ParseDuration(cfg.FailoverHeartbeatTTLCfg)
		if err != nil {
			fmt.Fprintf(os.Stderr, "unexpected error setting up failover-heartbeat-ttl: %v\n", err)
			os.Exit(1)
		}
	}
	if cfg.JobGCThresholdCfg != "" {
		cfg.JobGCThreshold, err = time.ParseDuration(cfg.JobGCThresholdCfg)
		if err != nil {
			fmt.Fprintf(os.Stderr, "unexpected error setting up job-gc-threshold: %v\n", err)
			os.Exit(1)
		}
	}
	if cfg.EvalGCThresholdCfg != "" {
		cfg.EvalGCThreshold, err = time.ParseDuration(cfg.EvalGCThresholdCfg)
		if err != nil {
			fmt.Fprintf(os.Stderr, "unexpected error setting up eval-gc-threshold: %v\n", err)
			os.Exit(1)
		}
	}
	if cfg.NodeGCThresholdCfg != "" {
...
func (cfg *Config) Validate() error {
	...
	if cfg.MinHeartbeatTTLCfg != "" {
		_, er := time.ParseDuration(cfg.MinHeartbeatTTLCfg)
		if er != nil {
			return fmt.Errorf("--min-heartbeat-ttl must be a time duration (set to %ds)", 5)
		}
	}
